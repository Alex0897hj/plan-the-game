...
/**
 * GamesMap — full-screen modal with a Yandex JS API map.
 * Shows markers for all upcoming games that have coordinates.
 * Clicking a marker navigates to the game page.
 */
...
export default function GamesMap({ games, onClose }: Props) {
  ...
  // Capture router in ref so ymaps event callbacks always have the latest version
  ...
  useEffect(() => {
    ...
    loadYmaps().then(() => {
      ...
      // Pick center: average of all pins, or Moscow as fallback
      const withCoords = games;
      const centerLat = withCoords.length
        ? withCoords.reduce((s, g) => s + g.lat, 0) / withCoords.length
        : 55.7558;
      const centerLng = withCoords.length
        ? withCoords.reduce((s, g) => s + g.lng, 0) / withCoords.length
        : 37.6173;
      ...
      // Global navigation handler called from balloon button HTML
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  }, []); // games list is stable after mount
